...
public class HealthRiServer extends Server {
    ...
    public void selectData(int i) {
        ...
        for (List<Attribute> attribute : d) {
            ArrayList<Attribute> copied = new ArrayList<>();
            for (int j = 0; j < i; j++) {
                copied.add(attribute.get(j));
            }
            copy.add(copied);
        }
        data = new Data(data.getIdColumn(), data.getLocalPresenceColumn(), copy);
    }
